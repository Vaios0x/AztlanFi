...
async function main() {
  console.log("🚀 Deploying AztlanFi smart contracts to local network...");
  ...
  try {
    ...
    const incentiveVault = await IncentiveVault.deploy(ethers.ZeroAddress, await remittanceToken.getAddress());
    ...
    const remittancePool = await RemittancePool.deploy(await exchangeRateOracle.getAddress());
